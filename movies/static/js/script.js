document.addEventListener('DOMContentLoaded', () => {
    ...
    async function addMovie(title, initialRating) {
        try {
            ...
            if (!response.ok) {
                throw new Error(result.message || result.error || 'Failed to add movie');
            }
    ...
    async function getMoviesInCategory(category) {
        const response = await fetch(api(`api/movies?user=${encodeURIComponent(currentUser)}&category=${encodeURIComponent(category)}`));
    ...
    async function handleComparison(choice) {
        ...
        if (choice === 'a') current.low = current.mid + 1;
        else if (choice === 'b') current.high = current.mid - 1;
        else current.low = current.high + 1;
    ...
    async function loadUserMovies() {
        try {
            const response = await fetch(api(`api/movies?user=${encodeURIComponent(currentUser)}`));
            const movies = await response.json();
